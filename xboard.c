...
int checkresult(S_BOARD *pos) {
    ...
    if (pos->fiftyMove > 100) {
     printf("1/2-1/2 {fifty move rule (claimed by Tundra)}\n"); return TRUE;
    }

    if (ThreeFoldRep(pos) >= 2) {
     printf("1/2-1/2 {3-fold repetition (claimed by Tundra)}\n"); return TRUE;
    }

	if (DrawMaterial(pos) == TRUE) {
     printf("1/2-1/2 {insufficient material (claimed by Tundra)}\n"); return TRUE;
    }
	...
	if(InCheck == TRUE)	{
	    if(pos->side == WHITE) {
	      printf("0-1 {black mates (claimed by Tundra)}\n");return TRUE;
        } else {
	      printf("0-1 {white mates (claimed by Tundra)}\n");return TRUE;
        }
    } else {
      printf("\n1/2-1/2 {stalemate (claimed by Tundra)}\n");return TRUE;
    }
	return FALSE;
}
...
void Console_Loop(S_BOARD *pos, S_SEARCHINFO *info) {
	...
	printf("Welcome to Tundra Chess version 0.1 in Console Mode!\n");
	...
	while(TRUE) {
		...
		if(!strcmp(command, "help")) {
			...
			printf("enter moves using algebraic notation (e2e4 d7d5 e4d5 d8d5 ... b7b8q)\n\n\n");
			continue;
		}
